Rails.application.routes.draw do
  ...
  namespace :api do
    namespace :v1 do
      ...
      resources :rosters, only: [:index, :create, :show, :destroy], defaults: { format: 'json' } do
        resources :participants, only: [:index, :create, :show, :destroy, :update], defaults: { format: 'json' }
      end
    end
  end
  root 'homepage#index'
